...
const components: MDXComponents = {
  ...
  h3: ({ children }) => (
    <h3 className="text-xl md:text-2xl font-bold text-slate-900 mt-8 mb-3 leading-snug">
      {children}
    </h3>
  ),
  h4: ({ children }) => (
  ...
  code: ({ children }) => (
    <code className="bg-slate-100 text-slate-900 px-1.5 py-0.5 rounded-[6px] text-sm font-mono">
      {children}
    </code>
  ),
  pre: ({ children }) => (
    <pre className="bg-slate-900 text-slate-100 p-4 rounded-[12px] overflow-x-auto my-6 text-sm font-mono">
      {children}
    </pre>
  ),
  table: ({ children }) => (
  ...
  th: ({ children }) => (
    <th className="bg-slate-100 border border-slate-200 px-4 py-2 text-left font-bold text-slate-900">
      {children}
    </th>
  ),
  td: ({ children }) => (
